 ...
 *
 * @ViewsStyle(
 *   id = "idc_iiif_serializer",
 *   title = @Translation("iDC IIIF Serializer"),
 *   help = @Translation("Display images as an IIIF Manifest."),
 *   display_types = {"data"}
 * )
 */
class IdcIiifSerializer extends StylePluginBase {
  ...
  protected function getTileSourceFromRow(ResultRow $row, $iiif_address, $iiif_base_id) {
    ...
    foreach ($this->options['iiif_tile_field'] as $iiif_tile_field) {
      ...
      ->getStorage('node')
      ->load($entity->get('field_media_of')->getValue()[0]['target_id']);

      $lang_entity_refs = $node->get('field_description')->referencedEntities();
      $english = null;
      ...
      $descriptions = $node->get('field_description')->getValue();
      ...
      $citable_url = $node->get('field_citable_url')->getValue()[0]["uri"];
